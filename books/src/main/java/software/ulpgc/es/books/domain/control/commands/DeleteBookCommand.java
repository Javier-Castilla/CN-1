...
public class DeleteBookCommand implements Command {
    ...
    public DeleteBookCommand(Input input, Output output, BookRepository repository) {
        this.input = input;
        this.output = output;
    ...
    @Override
    public void execute() {
        this.output.result(buildResult(this.repository.deleteBook(this.input.isbn())));
    }

    private Map<String, Object> buildResult(Book book) {
        Map<String, Object> result = new HashMap<>();
        if (book == null) {
            result.put("message", "Book does not exist");
        } else {
